  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        ...
        className="flex items-center justify-center gap-2 rounded-lg border border-blue-500/30 bg-blue-500/10 px-2.5 py-2 text-sm font-semibold text-(--foreground) shadow-sm transition-colors hover:bg-(--border) focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-500 sm:px-3"
      >
            ...
            {supportedChains.map((chain) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center justify-between gap-3 rounded-lg px-2 py-2 text-left text-sm transition-colors ${isSelected ? 'bg-blue-500/10 text-blue-500' : 'text-(--foreground) hover:bg-(--accent)'
                    }`}
                >
