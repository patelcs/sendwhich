import { useWallet } from '@/providers/WalletProvider';
import AccountMenu from './AccountMenu';
import ChainMenu from './ChainMenu';
import ConnectButton from './ConnectButton';
import LoadingDots from './LoadingDots';
import { Wallet } from 'lucide-react';

export function WalletConnection() {
  const { status } = useWallet();

  if (status === 'connected')
    return (
      <div className="ml-auto flex min-w-0 items-center gap-2 sm:gap-3">
        <ChainMenu />
        <AccountMenu />
      </div>
    );

  if (status === 'initialized' || status === 'disconnected')
    return (
      <div className="ml-auto flex min-w-0 items-center gap-2 sm:gap-3">
        <ConnectButton />
      </div>
    );

  return (
    <div className="flex items-center justify-center gap-2 rounded-lg border border-(--brand)/30 bg-(--brand)/10 px-5 py-2.5 text-sm font-semibold text-(--brand) shadow-sm shadow-(--brand)/15 transition-colors hover:border-(--brand)/50 hover:bg-(--brand)/20 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--brand) disabled:cursor-not-allowed disabled:opacity-70">
      <Wallet size={16} aria-hidden="true" />
      <LoadingDots />
    </div>
  );
}
